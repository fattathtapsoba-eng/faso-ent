...
import { Home, Users, DollarSign, Settings, BookOpen, Calendar, MessageSquare, Bell, ClipboardCheck, UserCog, UsersRound } from 'lucide-react';
...
export function BottomNav({ role }: { role: 'ADMIN' | 'TEACHER' | 'PARENT' | 'STUDENT' }) {
    ...
    const navItems = role === 'ADMIN'
        ? [
            ...
            { to: '/admin/parents', icon: UsersRound, label: 'Parents' },
            { to: '/admin/finances', icon: DollarSign, label: 'Finances' },
            { to: '/admin/teachers', icon: UserCog, label: 'Profs' },
            { to: '/admin/settings', icon: Settings, label: 'Paramètres' },
        ]
        : role === 'TEACHER'
            ? [
                { to: '/teacher/dashboard', icon: Home, label: 'Accueil' },
            ...
            : role === 'PARENT'
                ? [
                    { to: '/parent/dashboard', icon: Home, label: 'Accueil' },
